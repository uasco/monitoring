...
exports.getRainStationRainValues = catchAsync(async (req, res, next) => {
  ...
  //console.log(`values controller - rain values - ${client_id}`);
  //console.log('---------------------------------------------');
  if(subtype == 'r'){
  ...
  //console.log(`${client_id} : rainvalues : ${resultJson}`);
  //console.log('---------------------------------------------');
...
exports.getRainTotalsOfPastMonths = catchAsync(async (req, res, next) => {
  ...
  //console.log(`values controller - RainTotalsOfPastMonths - ${client_id}`);
  //console.log('---------------------------------------------');
  if(subtype == 'r'){
  ...
  //console.log(`${client_id} : raintotalofpastmonths : ${resultJson}`);
  //console.log('---------------------------------------------');
...
exports.getRainStartRainValues = catchAsync(async (req, res, next) => {

  const client_id = req.params.id * 1;
  const subtype = req.params.subtype;

  let rainStartTime = undefined;
  let rainStartRainValues = undefined;
  let apiResult = {};
  //console.log(`values controller - RainTotalsOfPastMonths - ${client_id}`);
  //console.log('---------------------------------------------');
  const stn = await Stn.findOne({client_id: client_id});
  if (stn != null)
    rainStartTime = stn.rain_start_time;
  rainStartTime = myDate.convert_mongodate_to_isodate(rainStartTime);
  console.log(`rainStartTime === ${client_id}   ->  ${rainStartTime}`);
  if(rainStartTime !== undefined){
    if(subtype == 'r' ){
      rainStartRainValues = await Values.getRainStartRainValues(client_id, channel_index_rain_total,rainStartTime);
    }else if(subtype == 'c'){
      rainStartRainValues = await Values.getRainStartRainValues(client_id, channel_index_rainc_total,rainStartTime);
    }
    let resultJson = JSON.stringify(rainStartRainValues);
    console.log(`${client_id} : rainStartRainValues : ${resultJson}`);
    console.log('---------------------------------------------');
    //console.log("new pars:::::::::::::::");
    //console.log(resultJson);
    resultJson = JSON.parse(resultJson);
...
exports.getLevelStationLastHours = catchAsync(async (req, res, next) => {
  const client_id = req.params.id * 1;
  //console.log(`values controller - LevelStationLastHours - ${client_id}`);
  //console.log('---------------------------------------------');
  const levelLastHours = await Values.getLevelLastHours(client_id, 74);

  var resultJson = JSON.stringify(levelLastHours);
  //console.log(`${client_id} : levelLastHours : ${resultJson}`);
  resultJson = JSON.parse(resultJson);
  ////////////////////////
  //console.log('resultJson["data"] ====');
  //console.log(resultJson);
...
exports.getClimaStationValues = catchAsync(async (req, res, next) => {
  ...
  //console.log(`values controller - ClimaStationValues - ${client_id}`);
  //console.log('---------------------------------------------');
  if (sensor === 'rainc') {
  ...
  //console.log(`${client_id} : ClimaStationValues : ${resultJson}`);
  //console.log('---------------------------------------------');
  resultJson = JSON.parse(resultJson);
  ////////////////////////
  //console.log('resultJson["data"] ====');
...
exports.getClimaMantagheiReport = catchAsync(async (req, res, next) => {
  ...
  let startTime = startDate;
  let endTime = endDate;
  let sensors_indexes = {'evp_a':channel_index_evp_a,'rainc_t':channel_index_rainc_total, 'tmp_l':channel_index_tmp_l,'tmp_n':channel_index_tmp_n,'tmp_x':channel_index_tmp_x, 'hum_l':channel_index_hum_l};
  let climaMantagheiReport=undefined;
  //console.log(`befor call: ${client_id}, ${startTime},  ${endTime}`);
  climaMantagheiReport = await Values.getClimaMantagheiReport(client_id,sensors_indexes,startTime,endTime);
  ...
  var resultJson = JSON.stringify(resultJson);
  //console.log(`mantaghei report : ${resultJson}`);
  var apiResult = {};
...
exports.getExcelClimaMantagheiReport = catchAsync(async (req,res,next)=>{
  ...
  for(let i=0; i<values[0].length;i++){
    let item = {};
    if(values[0][i] != null)
      item['day']=myDate.get_number_of_day_of_given_date(values[0][i]);
    if(values[1][i] != null)
      item['evp_a_6_30']=values[1][i];
    if(values[2][i] != null)
      item['evp_a_18_30']=values[2][i];
    ...
    if(values[5][i] != null)
      item['tmp_n_6_30']=values[5][i];
    if(values[6][i] != null)
      item['tmp_x_18_30']=values[6][i];
    if(values[7][i] != null)
      item['tmp_l_6_30']=values[7][i];
    if(values[8][i] != null)
      item['hum_l_6_30']=values[8][i];
    if(values[9][i] != null)
      item['tmp_l_12_30']=values[9][i];
    if(values[10][i] != null)
      item['hum_l_12_30']=values[10][i];
    if(values[11][i] != null)
      item['tmp_l_18_30']=values[11][i];
    if(values[12][i] != null)
      item['hum_l_18_30']=values[12][i];
