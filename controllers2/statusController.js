...
async function enoughEspentTime(stnID,statusType) {
...
exports.getRainStartStatus = catchAsync(async (req, res, next) => {
    ...
    let result = await Status.getRainStartStatus(client_id,channel_index_rain_total);
    ...
    if(result===1){//it's raining
        ...
        if( !st) {//rain has not started already
            ...
            let enough=await enoughEspentTime(client_id);
            if (enough) {
...
exports.getRainAlarm1Status = catchAsync(async (req, res) => {
    const client_id = req.params.id * 1;
    let result = await Status.getRainAlarmStatus(client_id,channel_index_rain_total,rain_alarm1_period,rain_alarm1_height);
    let resultJson = JSON.stringify(result);
    resultJson = JSON.parse(resultJson);
...
exports.getRainAlarm8Status = catchAsync(async (req, res) => {
    const client_id = req.params.id * 1;
    let result = await Status.getRainAlarmStatus(client_id,channel_index_rain_total,rain_alarm8_period,rain_alarm8_height);
    let resultJson = JSON.stringify(result);
    resultJson = JSON.parse(resultJson);
...
exports.getLevelAlarmStatus = catchAsync(async (req, res) => {
    const client_id = req.params.id * 1;
